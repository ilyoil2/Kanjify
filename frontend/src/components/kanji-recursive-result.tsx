    ...
    <div className={`flex flex-col ${isRoot ? "" : `ml-8 mt-4 border-l-2 border-slate-100 pl-6 relative before:absolute before:left-[-2px] before:top-0 before:w-2 before:h-2 before:bg-white before:border-l-2 before:border-t-2 before:border-slate-100 before:rounded-tl-lg`}`}>
      <div className="flex items-center gap-4 group">
        <div className={`
          ...
          ${isRoot 
            ? `size-14 rounded-2xl bg-slate-900 text-white shadow-xl shadow-slate-200 rotate-[-2deg] group-hover:rotate-0` 
...
export function KanjiRecursiveResult({ data, word, variant = "default" }: KanjiRecursiveResultProps) {
  ...
  return (
    <div className={`space-y-12 pb-20 ${isHistory ? "w-full" : ""}`}>
      {/* 1. Slim Premium Hero Section */}
      <div className="relative group">
        <div className="absolute -inset-1 bg-gradient-to-r from-slate-200 to-slate-100 rounded-[32px] blur-sm opacity-50" />
        <div className="relative bg-white border border-slate-200/60 rounded-[32px] p-10 shadow-sm overflow-hidden">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">
                  {isHistory ? "Archived record" : "Deep Analysis Output"}
                </span>
              </div>
              <h2 className="text-7xl md:text-8xl font-black text-slate-900 tracking-tighter leading-none">
                {word}
              </h2>
            </div>
            ...
            {data.word_info && (
              <div className="flex flex-col items-start md:items-end gap-4">
                <div className="text-right">
                  <p className="text-4xl font-black text-slate-900 tracking-tight">{data.word_info.meaning_ko}</p>
                  <p className="text-xl font-bold text-blue-600/80 mt-1">{data.word_info.reading_hiragana}</p>
                </div>
                <div className="flex flex-wrap gap-1.5 justify-end">
                  {rootChars.map(c => (
                    <div key={c} className="px-3 py-1.5 rounded-xl bg-white border border-slate-200/60 shadow-sm text-[12px] font-black flex items-center gap-2">
                      <span className="text-slate-900">{c}</span>
                      <div className="flex items-center gap-1">
                        <span className="text-slate-600">{data.nodes[c]?.meaning}</span>
                        <span className={themeText}>{data.nodes[c]?.reading}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="space-y-3">
            {data.examples && data.examples.length > 0 ? (
              data.examples.map((ex, idx) => (
                <div key={idx} className="group bg-white border border-slate-200/60 rounded-2xl p-5 shadow-sm hover:border-emerald-200 hover:shadow-md transition-all">
